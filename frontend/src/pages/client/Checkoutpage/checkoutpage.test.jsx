import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import '@testing-library/jest-dom';
import CheckoutPage from './checkoutpage';
import * as api from '../../../api/Api';

// Mock the API calls
jest.mock('../../../api/Api', () => ({
  cartApi: jest.fn(),
  createOrderApi: jest.fn(),
  fetchProductDetails: jest.fn(),
}));

describe('CheckoutPage Component', () => {
  beforeEach(() => {
    // Mock localStorage
    global.localStorage = {
      getItem: jest.fn(() => JSON.stringify({ _id: 'user123' })),
    };

    // Clear previous mock calls
    jest.clearAllMocks();
  });

  it('should display loading state initially', () => {
    api.cartApi.mockResolvedValue({ data: { success: true, cart: { products: [] } } });
    api.fetchProductDetails.mockResolvedValue({ data: { success: true, products: [] } });

    render(<CheckoutPage />);

    expect(screen.getByText('Loading...')).toBeInTheDocument();
  });

  it('should handle error state correctly', async () => {
    api.cartApi.mockRejectedValue(new Error('Failed to fetch cart items'));

    render(<CheckoutPage />);

    await waitFor(() => {
      expect(screen.getByText('An error occurred while fetching cart items')).toBeInTheDocument();
    });
  });

  it('should render cart items correctly', async () => {
    api.cartApi.mockResolvedValue({
      data: {
        success: true,
        cart: { products: [{ productId: 'prod123', quantity: 2 }] },
      },
    });
    api.fetchProductDetails.mockResolvedValue({
      data: {
        success: true,
        products: [{ _id: 'prod123', productName: 'Sample Product', productPrice: 100, productDescription: 'Description', productImage: 'image.jpg' }],
      },
    });

    render(<CheckoutPage />);

    await waitFor(() => {
      expect(screen.getByText('Sample Product')).toBeInTheDocument();
      expect(screen.getByText('Rs. 100.00')).toBeInTheDocument();
      expect(screen.getByText('Description')).toBeInTheDocument();
    });
  });

  it('should handle "Buy Now" button click correctly', async () => {
    api.cartApi.mockResolvedValue({
      data: {
        success: true,
        cart: { products: [{ productId: 'prod123', quantity: 2 }] },
      },
    });
    api.fetchProductDetails.mockResolvedValue({
      data: {
        success: true,
        products: [{ _id: 'prod123', productName: 'Sample Product', productPrice: 100, productDescription: 'Description', productImage: 'image.jpg' }],
      },
    });
    api.createOrderApi.mockResolvedValue({ data: { success: true } });

    const navigate = jest.fn();
    jest.mock('react-router-dom', () => ({
      useNavigate: () => navigate,
    }));

    render(<CheckoutPage />);

    await waitFor(() => {
      fireEvent.click(screen.getByText('Buy Now'));
      expect(api.createOrderApi).toHaveBeenCalledWith({
        userId: 'user123',
        items: [{ productId: 'prod123', quantity: 2 }],
        totalAmount: 200,
      });
      expect(navigate).toHaveBeenCalledWith('/order-confirmation');
    });
  });

  it('should handle invalid total amount', async () => {
    api.cartApi.mockResolvedValue({
      data: {
        success: true,
        cart: { products: [{ productId: 'prod123', quantity: 2 }] },
      },
    });
    api.fetchProductDetails.mockResolvedValue({
      data: {
        success: true,
        products: [{ _id: 'prod123', productName: 'Sample Product', productPrice: 'invalid', productDescription: 'Description', productImage: 'image.jpg' }],
      },
    });

    render(<CheckoutPage />);

    await waitFor(() => {
      fireEvent.click(screen.getByText('Buy Now'));
      expect(screen.getByText('Invalid total amount')).toBeInTheDocument();
    });
  });

  it('should handle failed order creation', async () => {
    api.cartApi.mockResolvedValue({
      data: {
        success: true,
        cart: { products: [{ productId: 'prod123', quantity: 2 }] },
      },
    });
    api.fetchProductDetails.mockResolvedValue({
      data: {
        success: true,
        products: [{ _id: 'prod123', productName: 'Sample Product', productPrice: 100, productDescription: 'Description', productImage: 'image.jpg' }],
      },
    });
    api.createOrderApi.mockResolvedValue({ data: { success: false, message: 'Failed to create order' } });

    render(<CheckoutPage />);

    await waitFor(() => {
      fireEvent.click(screen.getByText('Buy Now'));
      expect(screen.getByText('Failed to create order')).toBeInTheDocument();
    });
  });
});
